  ...
  async function loadData() {
    try {
      const [menuRes, combosRes, logoRes] = await Promise.all([
        fetch(`/api/menu?slug=${encodeURIComponent(slug)}&t=${Date.now()}`, {
          cache: "no-store",
          headers: buildHeaders() as HeadersInit,
        }),

        fetch(`/api/combos?slug=${encodeURIComponent(slug)}&t=${Date.now()}`, {
          cache: "no-store",
          headers: buildHeaders() as HeadersInit,
        }),

        fetch(`/api/company/logo?slug=${encodeURIComponent(slug)}&t=${Date.now()}`, {
          cache: "no-store",
          credentials: "include",
